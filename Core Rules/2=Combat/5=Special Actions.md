...
Reaction  
When a combatant enters or exits the range of your melee weapon or unarmed attack range (including ranged unarmed attack), you may make an attack with that method against the foe. You may only make a single such attack this way.  
You may also attempt to use a thrown weapon this way but the foe has a +4 bonus when such a weapon is used.
...
#### Counter Attack
Reaction  
If a creature attempted an attack against you and it was a critical failure, you peform a melee or thrown attack against them if they are within range.
...
#### Feint
Reaction  
An altering of the angle of your attack to allow you to change its course easily. In response to your own melee attack roll, regardless of the result, you may treat it as a critical failure for the purposes of other creatures' reactions. If a creature takes a reaction in response to the attack, you may perform a melee attack against that reacting creature with advantage if it is in range. Any reaction it takes still occurs if valid.  
If no creature performed a reaction in response to it, or you simply don't want to perform the attack against that reacting foe, you attack is resolved as it would normally (using the actuual result).  
...
#### Sunder
Reaction  
Performed in response to an attack you make, you instead intend to rend armor (or a shield), rather than maim the foe. If the attack is successful, you deal a point of damage to the armor. The effect of a critical hit still occurs. An attack that would qualify to deal damage armor this way still deals a point of damage to the armor. If it qualifies and the attack is successful, it allows you to choose whether you deal an additional point of damage to the armor or inflict a wound (essentially making this a normal attack) in addition. A narrator may opt to impose a defense penalty if performed against a creature with a shell or carapace in place of armor.  
If a foe has a shield but no armor, their shield becomes unusable as a result. If the foe blocks with a shield, and the sunder is successful- you may choose to have that shield rendered useless or to damage the armor.
