Grimoires are tomes filled with the details of magical processes. Each tome contains 3-5 spells and can provide a +1 bonus to spellchecks with (and +1 to saves against) the spells contained therein if one already is knowledgeable in the spell, exactly like the bonus for staves (any feature which pertains to that bonus pertains to this as well). Additionally, if wielded in two hands, it can be studied by one capable of casting spells to cast a spell contained within even if the caster does not know it- but this requires the casting time be multiplied by the spell's minimum tier +1 and does not allow the spell to be cast as any tier other than what it is recorded as within the grimoire.
