...
$6,5000 , 1.75 lbs.  
Chakram  
When the user presses attack with this weapon, or performs an aimed attack with it, the user may *take on 1d4+1 points of spell sickness to have two illusory shuriken which look identical head towards the target of the attack as well. One will inflict a psychic attack and the other a necrotic, but both benefit from any chakram weapon training the user possesses. The shuriken attacks are resolved in the order desired by the user*. If user has warrior weapon mastery with chakrams, the real shuriken will inflict a cutting attack if in optimal range instead of a chakram attack.
...
If name is known, the user may also simply choose to apply this to a single attack they make each round.
